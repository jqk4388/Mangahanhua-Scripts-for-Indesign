...
// Check if the "页码" layer exists, if not create a new layer named "SFX"
var targetLayer;
try {
    targetLayer = doc.layers.itemByName("SFX");
    targetLayer.name; // This will throw an error if the layer does not exist
} catch (e) {
    targetLayer = doc.layers.add({ name: "SFX" });
}
...
if (selectedItems.length === 0) {
    ...
    app.findGrepPreferences.findWhat = "~N"; // Grep expression for SFX
    ...
    for (var i = 0; i < selectedItems.length; i++) {
        if (selectedItems[i] instanceof TextFrame|| selectedItems[i] instanceof Group) {
