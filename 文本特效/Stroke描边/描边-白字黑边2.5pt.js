...
var defaultObject = {
    name: 'Stroke 2.5pt Black', // change this to something else if you want to use that specific name for something else
    strokeWeight: '2.5pt',
    strokeColor: 'Black',
...
// Set text color to Paper
function setTextColorToPaper(textFrame) {
    var paperColor = doc.swatches.itemByName("Paper");
    if (paperColor.isValid) {
        textFrame.texts.everyItem().fillColor = paperColor;
    }
}
...
for (var i = 0; i < selections.length && !hasErrors; i++) {
    ...
    if (!hasErrors) {
        if (selections[i] instanceof TextFrame) {
            setTextColorToPaper(selections[i]);
        }
