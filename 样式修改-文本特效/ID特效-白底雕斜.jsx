/*
ID特效-白底雕.jsx
为选中的文本框创建白底雕效果：修改字间距、描边与填充、复制生成多个偏移副本并应用外发光，再分组。

兼容性/限制：
 - 使用最基础的语法，不使用禁止的方法（如 trim, JSON.stringify, Array.isArray, indexOf, 字符串 "char"）
 - 所有功能封装为函数，使用 try-catch 捕获异常
 - 使用 InDesign 官方 ExtendScript API

主要步骤：
 1. 对每个选中的文本框：设置字距为 25（追踪值），适合文本框（如果有溢流文本）
 2. 为文本内容设置描边：黑色、0.25pt、斜接连接、斜接限制为 2 倍；填充为白色
 3. 在文本框上创建 10 个复制，每个副本相对于上一个偏移 x=-0.02mm, y=-0.02mm
 4. 将这 10 个副本分组，然后在原地复制一组，记录两个对象的 id
 5. 将复制的组移动到后方，并为其添加外发光效果（模式：滤色、100%不透明、方法：柔和、大小 0.4mm、杂色 0%、扩展 100%）
 6. 将十个文本框编组与外发光组副本一起再编组

注意：单位转换中，InDesign 的内部将使用点（pt），脚本中用毫米作为输入并转换为点（1 mm = 2.834645669 pt）。
*/
