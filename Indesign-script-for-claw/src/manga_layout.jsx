...
function main() {
    ...
    // ========== 步骤11: 辅助方框对齐 ==========
    if (config.ghostFrameAlignment && config.ghostFrameAlignment.enabled) {
        var step11 = executeStepWithTimeout("步骤11: 辅助方框对齐", function() {
            return runGhostFrameAlignmentWithResult();
        });
        stepResults.step11 = step11;
        
        if (!step11.success) {
            logMessage("[警告] 步骤11失败，跳过继续执行: " + step11.message);
            errorMessages.push("步骤11: " + step11.message);
        }
    } else {
        logMessage("[步骤11] 跳过辅助方框对齐（未启用）");
        stepResults.step11 = new StepResult(true, "跳过");
    }
    
    // ========== 步骤12: 保存文档（最后执行）==========
    if (config.output.saveDocument) {
        var step12 = executeStepWithTimeout("步骤12: 保存文档", function() {
            return saveDocumentWithResult();
        });
        stepResults.step12 = step12;
        
        if (!step12.success) {
            logMessage("[错误] 步骤12保存文档失败: " + step12.message);
            errorMessages.push("步骤12: " + step12.message);
        }
    } else {
        logMessage("[步骤12] 跳过文档保存（未启用）");
        stepResults.step12 = new StepResult(true, "跳过");
    }
...
// 主入口函数：执行辅助方框对齐流程
function runGhostFrameAlignmentWithResult() {
    try {
        if (!doc || !doc.isValid) {
            return new StepResult(false, "文档对象无效");
        }
        
        var ghostConfig = config.ghostFrameAlignment;
        
        // 1. 查找JSON文件路径
        var jsonPath = findGhostJsonPath(ghostConfig);
        if (!jsonPath) {
            return new StepResult(false, "未找到辅助方框JSON文件");
        }
        
        logMessage("使用JSON文件: " + jsonPath);
        var jsonFile = new File(jsonPath);
        if (!jsonFile.exists) {
            return new StepResult(false, "JSON文件不存在: " + jsonPath);
        }
        
        // 2. 读取JSON数据
        var jsonData = readGhostJsonFile(jsonFile);
        if (!jsonData) {
            return new StepResult(false, "无法解析JSON文件");
        }
        
        // 检查必要结构
        if (!jsonData['image_info']) {
            return new StepResult(false, "JSON文件缺少 image_info");
        }
        if (!jsonData['pages']) {
            return new StepResult(false, "JSON文件缺少 pages");
        }
        ...
        if (ghostConfig.createGhostFrames) {
            ...
            logMessage("生成辅助方框: " + ghostResult.created + " 个");
        }
        
        // 4. 对齐文本框
        var alignResult = { aligned: 0 };
        if (ghostConfig.alignTextFrames) {
            alignResult = alignTextFramesWithGhost(ghostConfig);
            logMessage("对齐文本框: " + alignResult.aligned + " 个");
        }
        
        // 5. 隐藏辅助图层（如果配置要求）
        if (ghostConfig.hideGhostLayerAfterAlign && ghostLayer && ghostLayer.isValid) {
            ghostLayer.visible = false;
            logMessage("已隐藏辅助图层");
        }
        
        return new StepResult(true, "辅助方框对齐完成", { 
            ghostFramesCreated: ghostResult.created, 
            textFramesAligned: alignResult.aligned 
        });
        
    } catch (e) {
        return new StepResult(false, "执行辅助方框对齐时出错: " + e.message);
    }
}

// 查找JSON文件路径
function findGhostJsonPath(ghostConfig) {
    // 1. 如果用户指定了路径，直接使用
    if (ghostConfig.jsonPath && ghostConfig.jsonPath !== "") {
        return ghostConfig.jsonPath;
    }
    
    // 2. 自动查找
    if (ghostConfig.autoFindJson) {
        // 从图片文件夹路径获取
        if (config.imageImport && config.imageImport.artFolderPath) {
            var artFolder = new Folder(config.imageImport.artFolderPath);
            if (artFolder.exists) {
                var folderName = artFolder.displayName;
                // 构建JSON文件名
                var pattern = ghostConfig.jsonFileNamePattern || "imgtrans_[folderName].json";
                var jsonFileName = pattern.replace("[folderName]", folderName);
                
                var jsonPath;
                if ($.os.indexOf("Windows") !== -1) {
                    jsonPath = artFolder.fsName + "\\" + jsonFileName;
                } else {
                    jsonPath = artFolder.fsName + "/" + jsonFileName;
                }
                ...
                logMessage("自动查找JSON未找到: " + jsonPath);
            }
        }
    }
...
// 读取JSON文件
function readGhostJsonFile(jsonFile) {
    try {
        ...
        logMessage("解析JSON文件失败: " + e.message);
        return null;
    }
}

// 生成辅助方框
function createGhostFramesWithResult(jsonData, ghostConfig) {
    ...
    try {
        ...
        // 2. 准备绿色颜色
        var colorName = ghostConfig.ghostColorName || "GhostGreen";
        ...
        // 3. 创建辅助图层
        var layerName = ghostConfig.ghostLayerName || "[輔助定位] 完整矩形框";
        ghostLayer = doc.layers.add({ name: layerName });
        ghostLayer.move(LocationOptions.AT_BEGINNING); // 最顶层
        ...
        logMessage("生成辅助方框时出错: " + e.message);
    }
...
// 创建辅助矩形框
function createGhostRects(page, blocks, layer, origW, origH, colorObj, label, ghostConfig) {
    ...
    var strokeWeight = ghostConfig.ghostStrokeWeight || 1.5;
    var created = 0;
...
// 获取页面上的图片名称
function getPageImageNameForGhost(page) {
    try {
...
// 在JSON pages中查找键
function findKeyInJsonPages(pagesObj, baseName) {
...
// 对齐文本框到辅助方框
function alignTextFramesWithGhost(ghostConfig) {
    var aligned = 0;
    
    try {
        var targetLayerName = ghostConfig.ghostLayerName || "[輔助定位] 完整矩形框";
        var localGhostLayer = doc.layers.itemByName(targetLayerName);
        
        if (!localGhostLayer.isValid) {
            logMessage("找不到辅助图层: " + targetLayerName);
            return { aligned: 0 };
        }
        
        // 获取需要处理的文本框
        var textFrames = getTextFramesForGhostAlignment(ghostConfig.alignRange);
        
        if (textFrames.length === 0) {
            logMessage("未找到需要对齐的文本框");
            return { aligned: 0 };
        }
...
// 获取需要对齐的文本框
function getTextFramesForGhostAlignment(range) {
    var textFrames = [];
...
// 判断两个物件是否相交
function isGhostIntersecting(obj1, obj2) {
...
// 对齐中心点
function alignGhostCenter(obj1, obj2) {
